...
fn place_active_blocks(mut commands: Commands, materials: Res<Materials>, mut active_block_positions: Query<&mut Position, With<ActiveBlock>>) {
    for mut position in active_block_positions.iter_mut() {
        if position.y == 0 {
            // Block has fallen to the ground. Place it.
            commands
            .spawn_bundle(SpriteBundle {
